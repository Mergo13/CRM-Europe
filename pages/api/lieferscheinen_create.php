<?php
// pages/api/lieferscheinen_create.php - lightweight create for convenience (uses posted fields or defaults)
...
try {
    ...
    if(empty($toInsert)){ if(in_array('nummer',$colNames)){ $toInsert[]='`nummer`'; $params[]='EX-'.time(); } if(in_array('kunde',$colNames)){ $toInsert[]='`kunde`'; $params[]='Demo Kunde'; } if(in_array('betrag',$colNames)){ $toInsert[]='`betrag`'; $params[]='0.00'; } if(in_array('status',$colNames)){ $toInsert[]='`status`'; $params[]='offen'; } if(in_array('created_at',$colNames)){ $toInsert[]='`created_at`'; $params[]=date('Y-m-d H:i:s'); } }
